...
public static class SpecimenUiPatch
{
    ...
    public static void Postfix(NCombatUi __instance, CombatState state)
    {
        ...
        if (player?.Character is not PlagueBringer) return;

        var energy = __instance.EnergyCounterContainer.GetChildren().OfType<NEnergyCounter>().FirstOrDefault();
    ...
    private static void Attach(NEnergyCounter energy, Player player)
    {
        ...
        if (counter == null)
        {
            ...
            Action<PowerModel> applied = power => { if (power is SpecimenPower) Refresh(); };
            Action<PowerModel, int, bool> increased = (power, _, _) => { if (power is SpecimenPower) Refresh(); };
            Action<PowerModel, bool> decreased = (power, _) => { if (power is SpecimenPower) Refresh(); };
            Action<PowerModel> removed = power => { if (power is SpecimenPower) Refresh(); };
